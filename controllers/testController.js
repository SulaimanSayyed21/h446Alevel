...
const handleTestRequest = async (req, res) => {
  ...
  console.log(`from sessin username is ${username}`);

  const { userName } = req.body.userName;
  console.log(`From test page username is ${req.body.userName}`);

  if ( true ) {
    try{
    console.log('Same perons is takng a test:', username);
...
const results = answers.map(answer => ({
  ...
  points: answer.points
    ...
    const updatedUser = await collection.findOneAndUpdate(
           ...
           { $push: { results: { $each: results } } }, // Add new 'testsTaken' array
